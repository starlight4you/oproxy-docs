...
Send a minimal request, then open **Usage** to confirm the model, endpoint, tokens, cost, latency, and User-Agent. For failures, check key status, group, Base URL, balance or subscription, key limits, and model monitoring. Use [Error Codes and Resolutions](/en/docs/oproxy/error-codes.html) for `429`, `503`, review, group, and balance issues.
